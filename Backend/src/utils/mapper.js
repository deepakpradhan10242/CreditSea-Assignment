function mapXmlToReport(root) {
  ...
  const summary = {
    ...
    last7daysEnquiries: safeNum(
      profile?.TotalCAPS_Summary?.TotalCAPSLast7Days
    ),
  };

  // --- ACCOUNTS ---
  const details = profile?.CAIS_Account?.CAIS_Account_DETAILS;
  const accountsArr = Array.isArray(details) ? details : details ? [details] : [];
  ...
  const accounts = accountsArr.map((a) => {
    ...
    return {
      ...
      holderName,
      holderAddress, // ✅ now an object (not string)
    };
  });
